...
app.post('/api/users/register', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      ...
      [userId, username, hashedOTP, role, firstName, lastName, dbEmail, dbPhone]
    );
...
app.get('/api/teacher/analytics', auth, async (req, res) => {
  try {
    ...
    // Get stats by class (through class_students junction table)
    ...
    const recentCompletionsResult = await pool.query(
      `SELECT 
         ...
         c.grade || c.section as student_grade,
...
app.get('/api/teacher/analytics/subject-modules', auth, async (req, res) => {
  try {
    ...
    // Get student count for specified grade/section (through class_students)
    ...
    if (section) {
      studentCountQuery = `
        ...
        INNER JOIN classes c ON cs.class_id = c.id
        WHERE c.grade = $1 AND c.section = $2 AND cs.left_at IS NULL
      `;
      ...
      studentCountQuery = `
        ...
        FROM class_students cs
        INNER JOIN classes c ON cs.class_id = c.id
        WHERE c.grade = $1 AND cs.left_at IS NULL
      `;
    ...
    // Get modules with test results filtered by class (through class_students)
    let modulesQuery;
    let modulesParams;
    ...
    const modules = modulesResult.rows.map(row => ({
      id: row.id,
      name: row.name,
      nameUz: row.name, // No separate uz/ru columns in modules table
      nameRu: row.name,
